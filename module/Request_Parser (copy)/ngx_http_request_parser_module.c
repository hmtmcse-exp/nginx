...
#include <ngx_core.h>
#include <ngx_http.h>
#include "ngx_http_request_parser_services.h"
...
static ngx_int_t ngx_http_request_parser_handler(ngx_http_request_t *r) {
    ...
    if (ngx_http_request_parser_post_arg(r, (u_char *) params_args, ngx_strlen((u_char *) params_args), &headerValue) == NGX_OK) {
        b->pos = headerValue.data;
        b->last = headerValue.data + headerValue.len;
        r->headers_out.content_length_n = headerValue.len;
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,"I am in post Request");
    } else {
        b->pos = args.data;
